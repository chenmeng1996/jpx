...
import cm.jpx.core.notify.NotifyService;
import cm.jpx.core.notify.NotifyType;
import cm.jpx.core.qcode.QCodeService;
import cm.jpx.core.system.SystemConfig;
import cm.jpx.core.task.TaskService;
import cm.jpx.core.util.DateTimeUtil;
import cm.jpx.core.util.IpUtil;
import cm.jpx.core.util.JacksonUtil;
import cm.jpx.core.util.ResponseUtil;
import cm.jpx.db.domain.*;
import cm.jpx.db.service.*;
import cm.jpx.db.util.CouponUserConstant;
import cm.jpx.db.util.GrouponConstant;
import cm.jpx.db.util.OrderHandleOption;
import cm.jpx.db.util.OrderUtil;
import cm.jpx.wx.task.OrderUnpaidTask;
import cm.jpx.wx.util.WxResponseCode;
    ...
    public Object submit(Integer userId, String body) {
        ...
        if (grouponRulesId != null && grouponRulesId > 0) {
            ...
            if (rules.getStatus().equals(GrouponConstant.RULE_STATUS_DOWN_EXPIRE)) {
                return ResponseUtil.fail(WxResponseCode.GROUPON_EXPIRED, "团购已过期!");
            }
            //团购规则已经下线
            if (rules.getStatus().equals(GrouponConstant.RULE_STATUS_DOWN_ADMIN)) {
                return ResponseUtil.fail(WxResponseCode.GROUPON_OFFLINE, "团购已下线!");
            }

            if (grouponLinkId != null && grouponLinkId > 0) {
                //团购人数已满
                if(grouponService.countGroupon(grouponLinkId) >= (rules.getDiscountMember() - 1)){
                    return ResponseUtil.fail(WxResponseCode.GROUPON_FULL, "团购活动人数已满!");
                }
                // NOTE
                ...
                if(grouponService.hasJoin(userId, grouponLinkId)){
                    return ResponseUtil.fail(WxResponseCode.GROUPON_JOIN, "团购活动已经参加!");
                }
                // （2）不允许参加自己开团的团购
